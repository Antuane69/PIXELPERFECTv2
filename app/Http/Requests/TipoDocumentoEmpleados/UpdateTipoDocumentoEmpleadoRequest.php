...
class UpdateTipoDocumentoEmpleadoRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $tipoDocumentoEmpleado instanceof TipoDocumentoEmpleado
            && ($this->user()?->can('update', $tipoDocumentoEmpleado) ?? false);
    }
    ...
    public function rules(): array
    {
        ...
        $data = $this->all();
        $hasRenewalInput = array_key_exists('es_renovable', $data)
            || array_key_exists('frecuencia_cantidad', $data)
            || array_key_exists('frecuencia_tipo', $data);
        $isRenewable = array_key_exists('es_renovable', $data)
            ? $this->boolean('es_renovable')
            : ($tipoDocumentoEmpleado instanceof TipoDocumentoEmpleado && $tipoDocumentoEmpleado->es_renovable);
        $requiresFrequency = $hasRenewalInput && $isRenewable;

        return [
            'nombre' => ['sometimes', 'required', 'string', 'min:2', 'max:120', $uniqueName],
            'es_renovable' => ['sometimes', 'boolean'],
            'frecuencia_cantidad' => [
                Rule::requiredIf($requiresFrequency),
                'nullable',
                'integer',
                'min:1',
                'max:65535',
            ...
            'documentos_aceptados' => ['sometimes', 'required', 'array', 'min:1', 'max:9'],
